...
cutoff = as.numeric(Sys.getenv("CUTOFF"))
...
mimosa_data <- function (brain_mask, FLAIR, T1, T2 = NULL, PD = NULL, tissue = FALSE,
                         ...
                         orientation = c("axial", "coronal", "sagittal"), cores = 1, verbose = TRUE, top_voxel_cutoff = 0.85) {
  if (verbose) {
  ...
  if(is.null(cand_mask)){
    top_voxels = oasis::voxel_selection(flair = mimosa_data$FLAIR,
                                        brain_mask = tissue_mask, cutoff = top_voxel_cutoff)
...
mimosa_training = readRDS(paste0(train_dir, "/", subjs[index], "/mimosa_model.RDS"))
...
writenii(mimosa_candidate_mask, paste0(train_dir, "/", subjs[index],
                                 "/top_voxels_",cutoff,"cutoff.nii.gz"))
...
writenii(probability_map, paste0(train_dir, "/", subjs[index],
                                 "/probability_map_",cutoff,"cutoff.nii.gz"))

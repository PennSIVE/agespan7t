...
for (subj in subjs) {
  print(subj)
  probmap = readnii(paste0('../data/scitran/cnet/7T-MS-agespan/', subj, '/probability_map.nii.gz'))
  ...
  for (thresh in thresholds) {
    ...
    dir.create(paste0('../data/scitran/cnet/7T-MS-agespan/', subj, '/thresholds'), showWarnings = FALSE)
    writenii(binmap, paste0('../data/scitran/cnet/7T-MS-agespan/', subj, '/thresholds/', thresh))
...
write.table(results, file = "../data/results.csv", sep = ",")
